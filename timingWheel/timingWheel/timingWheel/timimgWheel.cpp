...
//  timingWheel
//
//  Created by 雪之下雪乃 on 2020/5/31.
//  Copyright © 2020 付代超. All rights reserved.
//
...
public:
    ...
    tw_timer* add_timer(int timeout){
        ...
        int ticks = 0;
        /*下面根据待插入定时器的超时值计算它将在时间轮转动多少个嘀嗒
         后被触发，并将该嘀嗒数存储于变量ticks中。如果待插入定时器的超时值小于时间轮的槽间隔SI,则将ticks向上折合为1，否则就将ticks向下折合为timeout/SI*/
        if(timeout < SI){
            ticks = 1;
        }else{
            ticks = timeout / SI;
        ...
        if(!slots[ts]){
            printf("add tiemr, rotation is %d, ts is %d, cur_slot is %d\n", rotation, ts, cur_slot);
            slots[ts] = timer;
        }else{
            timer->next = slots[ts];
            slots[ts]->prev = timer;
    ...
    void del_timer(tw_timer* timer){
        ...
        /*slots[ts]是目标定时器所在的槽的头结点。如果目标定时器就是该槽的头结点，
         则需要重置第ts个槽的头结点*/
        if(timer == slots[ts]){
